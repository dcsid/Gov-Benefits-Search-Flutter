...
void _setupFlow(_MockSessionsService svc) {
  ...
  when(() => svc.create(any())).thenAnswer(
    (_) async =>
        api.SessionEnvelope(sessionId: 'sess-1', nextQuestion: flow[0]),
  );
...
void main() {
  ...
  test(
    'submit advances to the next question; finishes when none left',
    () async {
      final svc = _MockSessionsService();
      _setupFlow(svc);

      final container = ProviderContainer(
        overrides: [sessionsServiceProvider.overrideWithValue(svc)],
      );
      addTearDown(container.dispose);

      final session = await svc.create(const api.SessionCreateRequest());
      await container.read(
        screeningControllerProvider(session.sessionId).future,
      );
      final notifier = container.read(
        screeningControllerProvider(session.sessionId).notifier,
      );
  ...
  test('jumpTo returns to a previously answered question', () async {
    ...
    final session = await svc.create(const api.SessionCreateRequest());
    await container.read(screeningControllerProvider(session.sessionId).future);
    final notifier = container.read(
      screeningControllerProvider(session.sessionId).notifier,
    );
