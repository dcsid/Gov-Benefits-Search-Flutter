...
void main() {
  testWidgets('text renderer reflects value and emits changes',
      (tester) async {
    String last = '';
    await tester.pumpWidget(_wrap(
      TextInputRenderer(value: 'hi', onChanged: (v) => last = v),
    ));
  ...
  testWidgets('number renderer accepts only digits and dot', (tester) async {
    String last = '';
    await tester.pumpWidget(_wrap(
      NumberInputRenderer(value: '', onChanged: (v) => last = v),
    ));
  ...
  testWidgets('boolean renderer toggles selection', (tester) async {
    bool? value;
    Widget build() => _wrap(BooleanInputRenderer(
          value: value,
          onChanged: (v) => value = v,
        ));
  ...
  testWidgets('select renderer emits selected option', (tester) async {
    String? selected;
    await tester.pumpWidget(_wrap(SelectInputRenderer(
  ...
  testWidgets('multiselect renderer toggles values', (tester) async {
    ...
    Widget build() => _wrap(MultiSelectInputRenderer(
          options: const <QuestionOption>[
            QuestionOption(value: 'a', label: 'Apple'),
            QuestionOption(value: 'b', label: 'Banana'),
          ],
  ...
  testWidgets('date renderer renders the formatted value', (tester) async {
    await tester.pumpWidget(_wrap(DateInputRenderer(
