...
void main() {
  group('SessionsService', () {
    test('create posts to /sessions', () async {
      final dio = _MockDio();
      when(() => dio.post<Object?>('/sessions', data: any(named: 'data')))
          .thenAnswer(
      ...
      final out = await SessionsService(dio).create(
        const SessionCreateRequest(scope: 'state', stateCode: 'NY'),
      );
      expect(out.sessionId, 's-1');
      final captured = verify(() => dio.post<Object?>('/sessions',
    ...
    test('submitAnswers hits /sessions/{id}/answers', () async {
      final dio = _MockDio();
      when(() => dio.post<Object?>('/sessions/s-1/answers',
      ...
      await SessionsService(dio).submitAnswers(
        's-1',
        const SessionAnswersRequest(answers: <String, dynamic>{'state_code': 'NY'}),
      );
      verify(() => dio.post<Object?>('/sessions/s-1/answers',
    ...
    test('compare posts scenarios', () async {
      final dio = _MockDio();
      when(() => dio.post<Object?>('/sessions/s-1/compare',
          data: any(named: 'data'))).thenAnswer(
        (_) async => Response<Object?>(
